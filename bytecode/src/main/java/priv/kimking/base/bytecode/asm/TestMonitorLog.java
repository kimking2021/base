 ...
 * @date 2021/11/4
 */
public class TestMonitorLog extends ClassLoader {

    public static void main(String[] args) throws Exception {

        ClassReader cr = new ClassReader(MonitorMethod.class.getName());
        ClassWriter cw = new ClassWriter(cr, ClassWriter.COMPUTE_MAXS);

        ClassVisitor cv = new ProfilingClassAdapter(cw, MonitorMethod.class.getSimpleName());
        cr.accept(cv, ClassReader.EXPAND_FRAMES);

        byte[] bytes = cw.toByteArray();
        outputClazz(bytes);

        Class<?> clazz = new TestMonitorLog().defineClass("priv.kimking.base.bytecode.asm.MonitorMethod", bytes, 0, bytes.length);
        // 反射获取 main 方法
    ...
    static class ProfilingMethodVisitor extends AdviceAdapter {
        ...
        protected void onMethodEnter() {
            ...
            mv.visitInsn(IASTORE);
            mv.visitMethodInsn(INVOKESTATIC, "priv/kimking/base/bytecode/asm/MonitorLog", "info", "(Ljava/lang/String;[I)V", false);
        }
    ...
    private static void outputClazz(byte[] bytes) {
        ...
        try {
            String pathName = TestMonitorLog.class.getResource("/").getPath() + "AsmTestMonitorLog.class";
